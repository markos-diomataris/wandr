from utils.pytorch3d_transforms import (
...
def root2heading(R_pelvis, in_format: str="aa"):
    R_z = get_z_rot(R_pelvis, in_format=in_format)
    heading = -R_z[..., :, 1]  # take second column of rotation matrix
    return heading

def flip_motion(orig_motion_dict: dict):
    # copy the orig_motion_dict
    motion_dict = orig_motion_dict.copy()
    # check that motion_dict has "body_pose", "body_orient", "trans", "joint_positions"
    assert 'rots' in motion_dict.keys()
    assert 'trans' in motion_dict.keys()
    trans = motion_dict['trans'].clone()
    pose = motion_dict['rots'].clone()
    if 'joint_positions' in motion_dict.keys():
        joints = motion_dict['joint_positions'].clone()
    ...
    motion_dict['rots'] = pose
    motion_dict['trans'] = mirrored_trans
    if 'joint_positions' in motion_dict.keys():
        joints[..., 0] = -joints[..., 0]
        joints = joints[:, SMPL_JOINTS_FLIP_PERM]
        motion_dict['joint_positions'] = joints
    if 'target_bone' in motion_dict.keys():
        tb = motion_dict['target_bone']
        tb = tb.replace('Right', 'Left')\
        if tb.startswith('Right') else tb.replace('Left', 'Right')
        motion_dict['target_bone'] = tb
    return motion_dict


def batched(f, input: Tensor, d: int):
    alphabet = [chr(i) for i in range(97, 123)]
    # flattent all except last d dimentions
    input_flat = rearrange(input, f"{' '.join(alphabet[:input.ndim])} -> ({' '.join(alphabet[:d])}) ...")
    output_flat = f(input_flat)
    pass