...
# def generate_triangular_attention_mask(sequence_length, k):
#     """
#     Generate a triangular attention mask where each token can attend to at most k tokens before it.

#     Args:
#     - sequence_length (int): Length of the sequence.
#     - k (int): Maximum number of tokens each token can attend to before it.

#     Returns:
#     - torch.Tensor: Triangular attention mask with shape (sequence_length, sequence_length),
#                     where mask[i, j] = 1 if token j is allowed to attend to token i, and 0 otherwise.
#     """
...
def tri_att_mask(sequence_length, past_length):
    """
    Generate a triangular attention mask where each token can attend to at most k tokens before it.
    ...
    """
    # Create a 1D tensor representing positions
    positions = torch.arange(sequence_length)
    
    # Create a 2D tensor where each row represents the position and each column represents the offset
    # from the current position
    offsets = positions.unsqueeze(0) - positions.unsqueeze(1)
    
    # Create a mask where each element is False if the offset is between 0 and -(k-1) (inclusive)
    mask = (offsets > 0) | (offsets < -past_length)
